...
func TestGoldenChatToolsCanonicalAndStreamContract(t *testing.T) {
	t.Parallel()
	...
	if err := json.Unmarshal(payload, &identity); err != nil || identity.User != "caller_fixture_01" {
		t.Fatalf("fixture user identity = %q, %v", identity.User, err)
	}
	...
	stream := codec.NewStream("chat_fixture_response_01", request.Model)
	start, err := stream.Start()
	if err != nil || len(start) != 1 {
		t.Fatalf("start = %q, %v", start, err)
	}
	startPayload := decodeChatFixtureFrame(t, start[0])
	if startPayload["id"] != "chat_fixture_response_01" || startPayload["model"] != request.Model {
		t.Fatalf("start identity = %#v", startPayload)
	}
	...
	if err != nil || !done || len(frames) != 3 || string(frames[2]) != "data: [DONE]\n\n" {
		t.Fatalf("tool stream = %q, %v, %v", frames, done, err)
	}
	toolPayload := decodeChatFixtureFrame(t, frames[0])
	choice := fixtureMap(t, fixtureSlice(t, toolPayload["choices"])[0])
	delta := fixtureMap(t, choice["delta"])
	encodedCall := fixtureMap(t, fixtureSlice(t, delta["tool_calls"])[0])
	function := fixtureMap(t, encodedCall["function"])
	if encodedCall["id"] != call.ToolCallID || function["name"] != call.ToolName {
	...
	finish := fixtureMap(t, fixtureSlice(t, decodeChatFixtureFrame(t, frames[1])["choices"])[0])
...
func decodeChatFixtureFrame(t *testing.T, frame []byte) map[string]any {
	t.Helper()
	line := strings.TrimSpace(strings.TrimPrefix(string(frame), "data: "))
	var payload map[string]any
	if err := json.Unmarshal([]byte(line), &payload); err != nil {
		t.Fatalf("invalid SSE JSON %q: %v", frame, err)
	}
	return payload
}
