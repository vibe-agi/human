...
管道是认真的：全链路 fail-closed、逐字节重放、durable outbox、90 个 formal gate、跑真实 OpenCode CLI 的故障注入门。感兴趣看 [docs/](docs/)。
...
远程部署就两条命令：服务器上 `human gateway --listen :8080`（前面放 TLS），人的机器上 `human worker --gateway wss://.../internal/v1/worker/ws`。
