...
当前公共核心位于 `llm.Service`，三方言 codec 在 `llm/builtin`，HTTP adapter 在
`llm/callerhttp`，人侧领域与浏览器分别在 `workerkit`、`web`。`human local` 只组合
这套公共栈；`internal/completion` 是仍供独立远程兼容产品使用的旧实现，不再是本地或
新嵌入的架构真相。OpenCode 1.17.18 的 Chat/Workspace 与两轮 resume 已用真实 CLI
跑通；Codex Responses 的 Basic 函数闭环和 Anthropic Messages 的基础闭环已有验证。
“三种 wire codec 已实现”不自动外推为每个 harness 的 Workspace/Tasks/Live Workspace
都已验收，能力继续按具体 profile 记账。
...
| G-18 | Responses API stream/aggregate codec、严格控制字段、serial/namespace/hosted/reasoning/custom tool 边界与 golden fixture 已实现；Codex 0.145.0 已在公共 local 栈真实通过 RemoteTools 函数闭环与 exact freeform `apply_patch` Workspace 正常路径，其 Workspace 故障恢复仍由 M0 实测裁决 | M0 |
...
| G-21 | 公共 Go API：`llm.Service` 提供正确性内核与 Store/策略 ports；`callerhttp`/`workerws` 提供 transport；`workerkit` 提供人侧领域 ports；`local` 组合 loopback + SQLite + Web；CLI 为薄装配 | M1 |
...
| T-06 | Command：可替换 exact profile 在 caller 声明匹配 schema 时映射 Claude `Bash`、OpenCode `bash`、Codex `exec_command`；只生成 tool call，由客户 Agent 执行，绝不本地执行；通用 JSON 保留为其它声明工具的高级入口 | M1 |
...
| T-09 | Tasks：可替换 exact profile 从 caller 上下文恢复并编辑客户 Agent 的计划；OpenCode/Codex 同步全量列表，Claude 使用 `TaskCreate`/`TaskUpdate`/`TaskList` 生命周期；无兼容工具或 schema 漂移时专用面板禁用，不生成 Human 自用 Todo 或文本假同步 | M1 |
