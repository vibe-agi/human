...
- 根包已提供 `human.NewLLM()` / `human.NewAgent()`。前者是 `gateway.Open` 的薄 facade，不启动 listener/TUI；后者打开独立单 owner SQLite Agent 领域，不复用 completion request/response。Agent 已覆盖 authority-qualified Context/Workspace、同 Context 并行 Task、多轮 input-required、终态不可重开、分页历史、命令 replay/revision CAS、不可变 Artifact、final 原子发布、取消/失败 discard，以及受信 receipt 驱动的 Workspace confirmed-head CAS。Worker mutation 在同一 SQLite commit 内复验无墙钟 lease 的 worker/fence/revision；`FenceLease` 显式撤销，`ClaimLease` 在同一事务中选取最早可领 Task、增加 fence、记录不可变 grant 历史和精确 command receipt；没有以超时假装 fencing。
- 公共 `a2a` 包已实现官方 A2A 1.0 HTTP+JSON caller adapter。Agent Card 以标准 well-known 路径暴露，其余操作先认证，且只用 principal 派生 Authority，不信任 body tenant。已覆盖 send/get/list/cancel、GET/POST subscribe（归一到同一实现）、SSE streaming、`input-required` 后多轮续接、exact message retry、Workspace/Apply Receipt extension negotiation 与 body/card 合同校验。这是 caller transport，不是远程 Human worker transport。
- `workspace` 公共包已有 transport-neutral `ApplyIntent` / `CASApplier` 和单 owner SQLite apply journal：调外部 CAS 前先落 `pending`，exact retry 只重放终态，重启发现 pending 或 applier 返错会终结为 `indeterminate`，而不盲目重执行副作用。A2A Artifact → apply journal → authenticated apply-receipt → Agent confirmed head 的垂直切片已有回归。宿主仍必须实现真实文件树的 CAS applier；HumanLLM 尚未接入同一全局 revision chain，也没有把逐文件 callerfs 冒充多文件原子 apply。
...
3. **HumanAgent 远程 worker 与产品面**
   - commit-time 无时钟 lease/fence、原子 `ClaimLease`、官方 A2A 1.0 caller adapter 和 caller apply journal 垂直切片已完成，不再列为待办。
   - 尚需独立的 Agent worker transport：把 claim/grant 发给远程 worker，用 durable outbox 传回 command，并在 ACK/NACK 前完成领域 commit；不得把 HumanLLM completion WebSocket DTO 直接暴露成 Agent 协议。
   - 尚需官方 HumanAgent TUI/worker 装配，以及一个真实 caller 宿主的文件树 CAS applier 验收；当前只能宣称可嵌入 A2A caller 面和 apply 日志机制，不能宣称已有现成的远程人类 worker 产品或通用多文件原子落盘。
   - HumanLLM 的增量 mirror intent 与 HumanAgent Artifact 仍未接到同一条 workspace revision chain；当前只有 Agent 使用公共值类型和 caller journal，不能把目标架构写成已共享。
