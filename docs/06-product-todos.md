...
- `human local backup / verify-backup / restore` 已形成离线灾备闭环：按 canonical path 排序全持有 gateway/outbox/state owner lock，排除 local、独立 worker、gateway/token-admin 并发写；SQLite 用 source quick-check + `VACUUM INTO` + snapshot/staging/installed quick-check 吸收 sidecar 已提交状态；v1 archive 对固定 core 与 caller mirror/baseline 逐项绑定 mode/size/SHA-256，并交叉验证 active credential secret 与 gateway token row、gateway identity/worker subject 与全部 outbox/state correctness row。restore 默认拒绝非空目标，`--force` 经同目录 staging、持久 journal、old/new rename 整套提交，同时清除旧 sidecar；换 canonical DB 路径时 staging 内事务重绑 transport identity，不重写旧 in-flight workspace/root。任一崩溃边界由 `--resume` 收口且普通 local 启动 fail-closed。mirror symlink/特殊节点显式列入 skipped、不跟随。
