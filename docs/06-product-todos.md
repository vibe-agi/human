...
- 新 `llm` package 公开 Store、Codec、Caller/Worker Transport、Protector、Router/Policy ports；官方 `llm/sqlite`、`llm/callerhttp`、`llm/workerws` 与 `protect/aead` 都是可替换 adapter，并由完整 custom-framework 示例验证。旧 `gateway` package 继续提供 turnkey handler/token/恢复，`worker` 暴露 Bubble Tea model，`local` 负责 loopback 组合。官方持久 adapter 当前只实现 SQLite，但 `agent.Store` / `llm.Store` 已是公共第三方 driver contract。
