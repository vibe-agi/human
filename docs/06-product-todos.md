...
项目内部的可重复 network/service fault matrix 用 `make fault-test`；它同时运行旧 gateway/workerclient 网络矩阵、新 LLM/Agent 的 SQLite/Memory Store 语义矩阵、两套 worker Journal 的 release/reopen 与 Memory abandon 套件，以及 Agent/LLM SQLite Store、SQLite Journal、示例 custom Store 的无 `Release` 子进程退出恢复测试。对并发时序做重复抽样用 `make fault-test FAULT_COUNT=3`。安装了精确 OpenCode 1.17.18 后，完整产品链用 `make real-opencode-tui-test REAL_COUNT=3`，三断点真实网络恢复门用 `make real-opencode-network-test REAL_COUNT=3`；后者默认每个断点 `REAL_NETWORK_DROPS=5`。故障点、证据边界和 outbox 人工处置见 [08 部署与运维](08-operations.md)。
