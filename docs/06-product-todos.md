...
- 真实 OpenCode 1.17.18 CLI gate 已完成：从空 Human mirror 走与 `:pull native.txt` 相同的 bootstrap 生成与 hydration-intent 路径，修改 mirror，生成绝对路径原生 `edit`，CLI 实际执行并回传成功 result，Human reconcile 后继续同一工具循环里的 `bash + todowrite`、final；同一 OpenCode session 在旧 task terminal 后的下一顶层 user turn 正确建立新 Human task。这个自动化 gate 复用生产 mirror/hub 路径，**没有经过 Bubble Tea 键盘输入层**；完整 TUI 手工链仍在下方待验收。
...
- Reply/Command/Tasks 草稿和最多 32 个 parked continuation 默认写入 worker-local SQLite；仓库测试已证明草稿与单个 continuation 跨重启恢复，以及坏 state 记录逐条隔离且不阻断健康记录。多个 parked continuation 的整体跨重启门仍在下方。
...
不再跑更长 soak；本轮只验网络不稳定、服务异常和 Live Workspace 产品体验。

1. **真实 OpenCode + 完整 TUI 保存链**
   - 在隔离 fixture 中从 TUI 输入 `:pull`、修改 mirror 并保存；分别验默认 preview/confirm 与 `--workspace-auto-send`。CLI/worker 级真实 E2E 和上述 TUI 内部不变量已通过，此项只补 Bubble Tea 人工输入到真实 CLI 的完整用户路径。
   - 观察 OpenCode CLI 展示 bootstrap、原生 edit、result、下一 completion、Tasks/bash 与 final；Human 侧 baseline 必须只在成功 result 后推进。无 save-ahead 时 Review 归零，有 save-ahead 时新 diff 必须保留。
   - 验安全 warning/block 会停，OpenCode 无-CAS warning 可见；外部编辑导致 oldString 失配时必须响亮失败、重新取可信 baseline。

2. **真实 OpenCode 网络矩阵**
   - caller 在响应前、首帧后、progress 后掉线并重复失败至少 5 次；记录 CLI 是否重试、是否保留 `X-Session-Id`、是否真正复用同一语义请求。
...
3. **真实运维可见性**
   ...
   - 补 durable worker outbox 单条损坏时的 operator-visible 告警/隔离或修复路径；worker-state 坏记录隔离、草稿与单 continuation 重启恢复已有自动化，只剩多个 parked continuation 的整体跨重启门。
   - 固化一份可重复的 fault matrix：故障点、失败次数、恢复顺序、task/key、tool-call ID、数据库计数与用户可见结果。
