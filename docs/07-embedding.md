...
| `local` | loopback caller endpoint、`llm.Service`、SQLite、进程内 workerkit 与官方 Web 的基础组合 | 把 `BaseURL()` / `CallerToken()` 交给 Agent；或通过 `Service()` 调显式 maintenance/status API；跨重启 token 由宿主以 mode `0600` 等方式持久化 |
| `workerkit` | headless 人侧状态机、Snapshot/Notifications/命令及 Wire/StateStore/Mirror/Observer ports | 选择进程内或远程 Wire、持久化、镜像与自有 UI；官方 Web 只是一个投影 |
| `gateway` | 旧远程兼容产品的 completion/worker WebSocket 一体化服务 | 新嵌入优先使用 `llm.Service` ports；gateway 仍由远程 CLI 产品独立维护 |
...
桌面一体化使用 `local.Open`；它与根 `NewLLM` 使用同一个公共 core，不再内嵌旧
gateway 或 worker WebSocket。远程兼容 CLI 的 `gateway.Open` 是独立产品线，不定义
新库边界。

`local.Open` 适合桌面应用把“人 = LLM”直接放进本机进程。最小示例见 [`examples/embed-local`](../examples/embed-local/main.go)：`local.DefaultConfig()` 把 service DB 与 workerkit state 放在 OS 用户私有数据目录并按真实 workspace 隔离，不会默认写进客户仓库；mirror 是明确的用户可见目录。人侧 worker 在进程内，所以没有 worker token 或 outbox。未提供 `ExistingCallerToken` 时 `Open` 生成只存在内存的 caller token；需要跨重启复用时，宿主负责把 `Credentials().CallerToken` 安全持久化，再在下次 Open 传回 `ExistingCallerToken`。CLI 的参考实现使用单一 mode `0600` credential file。`Service()` 返回 Local 拥有的公共 core，宿主可在 Local 存活期调用 Status/RunExpiry/RunRetention，但不能单独关闭它。

```sh
go run ./examples/embed-local
```

`workerkit.Open` 可以放在任意宿主进程中。进程内使用
`workerkit.WrapConnection(service.OpenWorker(...))`；远程使用 `llm/workerws.Client`
adapter。StateStore、Mirror 和 Wire 都是借用的 ports，`Worker.Shutdown` 只停止领域循环，
资源关闭顺序仍由宿主负责。官方 `web.Server` 同样借用 Worker。

`web.Config.PlanProfiles` 是浏览器 Tasks/Plan 面板的可替换 SPI。nil 使用
`web.OfficialPlanProfiles()`：只对仓库真实门冻结的 exact harness/version 与 behavioral
JSON Schema 启用 OpenCode/Codex 全量计划或 Claude task lifecycle；description/title 这类
注解变化不影响匹配，新增字段、约束或 enum 会 fail closed。自定义 harness 可实现
`web.PlanProfileResolver` 返回规范化 list/task-lifecycle profile；语义更复杂时应替换整个
人侧 UI，而不是让基础 Web 猜 schema。这个 profile 只负责交互投影，工具 authority 仍由
caller 声明、`llm.ToolAuthorizer` 和 caller Agent 自身权限共同决定。

`web.Config.CommandProfiles` 对命令面板提供同样的替换点。nil 使用
`web.OfficialCommandProfiles()`，只在 Claude Code 2.1.217 `Bash`、OpenCode 1.17.18
`bash` 或 Codex 0.145.0 `exec_command` 的完整 behavioral schema 精确匹配时启用；schema
漂移或未知版本会隐藏专用面板，通用 declared-tool JSON 入口仍在。官方 profile 不把
Agent cwd 不进入 Task，也不发送给浏览器：命令默认在调用方 Agent 自己的当前 workspace
执行。自定义 `web.CommandProfileResolver` 可声明不同的工具名以及
command/description 字段；profile 只构造 tool call，Web 和 worker 都不会执行命令。

文件型扩展只接收 Human 主机路径：`fsmirror.Config.Root` 是基础目录，
`SessionMirror.PrepareSession` 可把某个会话绑定到 Human 已有 repo。builder 收到的
`Change.Path` 已是项目相对路径，不能拼接或猜测 Agent 绝对 root。官方 `fsmirror` 还要求
`workerkit.WorkspaceScope{Caller, WorkspaceKey}` 与会话 `HumanWorkspace.ID`，跨 scope/
会话投递 fail closed；自定义 Mirror 可以有自己的多 workspace 路由，但必须保持同等隔离。
...
- `local.Local` 拥有两个 loopback listener、caller transport、Service、workerkit、Web、Mirror 与内建 StateStore；`Close` 幂等并按逆依赖顺序拆除。传入的 custom StateStore 只借用、不由 Local release。
- `workerkit.Worker` 借用 Wire、StateStore 与 Mirror；`Shutdown` 停止领域循环和 adapter notice pump，不替宿主关闭这些 ports。
