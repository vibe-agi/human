...
3. **执行幂等 + tool-call 对账**：每个 tool_call 有唯一 ID；Remote tools / Workspace 在整个 task 内也禁止不同 event 复用同一 ID，并在 durable step 前拒绝。`human-shim@1` 以 `(caller_id, task_id, tool_call_id)` 记 SQLite ledger；文件型 ledger 有持有至 `Close` 的跨进程独占 owner lock，第二个 shim 不会把首进程的 live `pending` 提前收口。正常完成后重启仍只重放原 result，崩溃遗留的 `pending` 则收口为可重放的 `execution_outcome_indeterminate`；当前进程两次持久化都失败时，同 ID 后续只重试终态提交而不重跑工具。所有歧义都要求核对工作区后换新 ID，绝不盲目重跑可能已发生的副作用。回流 result 按 ID 匹配，成功推进 baseline，失败/缺失保留未确认 diff。Live Workspace 用明确的两阶段 save-ahead：先保存 exact event 的 pending row，再持久记录 reviewed mutation、exact call ID/digest、base fingerprint 与已发送内容，最后把同一 exact pending payload 标成 `delivery_intent_recorded=true`；只有该 phase 写成功才进入 durable outbox。重启无法判定同一 event 是否已在 crash 前完成 outbox Put，所以 recovered pending 的 assignment 按原字节冻结，任何变化都不得重写 state/outbox。准备/确认失败则先原子写 terminal discard tombstone 并删除 intent，再删除 pending row，重启不会复活已丢弃 call。discard 与 result reconcile 以同一锁序串行：真实 result 先完成就保留其 digest，discard 先完成则迟到 result 只成为该 call 的显式失败、不能推进 baseline。拒绝 finalizer 完成 mirror cleanup、pending delete 与 rejected-inbox 确认前，只关闭同一稳定 task/session scope 的发送入口；无关 scope 可继续服务。高级工具草稿以旧 `event_id + 行号` 确定性生成新的 call ID，不复用已 tombstone 的旧 ID。mirror 原子状态文件在临时文件 fsync、rename/link 后还会 fsync 父目录，新建目录链逐层同步，rename 对账删除 source 后也同步其父目录（Windows 使用平台文件系统语义）；这只保证 Human 自己的持久边界，不扩写成客户工具副作用 exactly-once。迟到 result 只确认对应 delivery intent；若 Human 已保存更领先的草稿，baseline 只推进到已发送版本，新 diff 保留。**OpenCode 原生工具没有 shim ledger/remote SHA 保证**：该 intent 保证 Human 不会确认错本地版本，不证明客户侧工具 exactly-once 或远端内容 CAS。
