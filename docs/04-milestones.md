...
Live Workspace 是主线的一部分：Human 保存 → exact profile 生成客户 Agent CLI 原生 edit/write/bash/tasks → result 回流 → continuation/reconcile。OpenCode 的 2h 持续流结果已归档，当前**不再追加更长挂门**；M0 可靠性门只看 caller、worker、gateway/SQLite 各自断线/重启，以及三方重叠故障的可恢复性。

本轮 RC 的可交付目标固定为 **OpenCode 1.17.18、单机 `human local`、单专家/单工作区**。Codex、Claude、其它 OpenCode 版本以及远程多 worker/多租户属于 M2 扩展证据，不阻塞这个 RC；反过来也不能把三方言 codec、Codex Basic gate 或内部故障测试外推成这些扩展已经兼容。

## 当前落点：RC 单机路径已有证据，扩展门继续补

| 范围 | 已有证据 | 仍缺证据 |
|---|---|---|
| M0/M1 核心 | 三方言 stream/aggregate codec、持久循环、adapter/shim、Live Workspace/TUI；OpenCode 1.17.18 Basic 工具闭环及 exact Workspace `空镜像 → :pull → edit/result → bash/tasks → final → 同 session terminal 后下一 user turn 新 task` 已真实通过；默认保存后的 fsnotify review、preview/confirm 也已由真实生产 TUI 门覆盖；Codex 0.144.5 Responses Basic 串行 `exec_command → function_call_output → final` 已真实通过 | opt-in auto-send 仍只有内部安全不变量；Codex Workspace/Tasks/Live Workspace；Claude 真实工具闭环；扩展产品任务 |
| 故障恢复 | 自动故障注入已覆盖 caller 同 key 5 次断流、worker 冷启动/5 次抖动/半开检测、gateway + SQLite 重启与三方重叠掉线；Workspace 正式测试再覆盖离线原生 edit、gateway/SQLite 与 worker outbox 重启、并发 replay、result continuation 与 save-ahead diff；真实 OpenCode 三断点各连续掉线 5 次并在第 6 次以同 body/session/idempotency 恢复；Codex 0.144.4 黑盒已确认 500/响应前断 TCP 的 5 次重试与 turn metadata | 真实 OpenCode 不同进程恢复顺序与用户可见状态；Codex partial SSE；超过 `max_pending` 的明确过期 UX；证书轮换；多 worker |
...
OpenCode 1.17.18 的 Basic/Chat 与 exact Workspace 均已跑通。Basic 无稳定身份时每次 completion 独立；exact Workspace 以原生 `X-Session-Id` 和截止最新 user 消息的 canonical 历史生成 candidate task，但同一 caller/workspace/exact harness/session 的唯一非终态 task 会覆盖 candidate。clarification、followup、tool call 与 result continuation 因而保持同 task，只有 terminal 后下一顶层 user 才新建 task。request key 直接由 caller/workspace/harness session 与两种请求摘要派生，不含可变 task_id。Codex CLI 0.144.4 的响应前重试/turn metadata 黑盒与 0.144.5 的真实 Responses Basic 工具闭环也已分别通过；尚未证明的是 Codex partial SSE 恢复和 Workspace/Tasks/Live Workspace。下表是故障证据的裁决维度；其中 OpenCode 三个真实网络断点已过，真实多进程恢复顺序仍待观察：

| 维度 | 验证 |
|---|---|
| 网络 | caller 在 200 前/首帧后/部分进度后断开；worker 冷启动离线、反复抖动、半开连接与 ACK 丢失 |
| 服务 | gateway 在 admitted/200/event step/decision/receipt 窗口重启；worker 进程重启并复用 outbox |
| 重叠故障 | caller、worker、gateway 三方同时离线，按不同顺序恢复；事件不丢、不重、不阻塞后续 session |
...
> 前置：终端 A 按 **OpenCode 1.17.18 exact Workspace** 配置 base_url/token、静态 workspace/profile headers 与绝对 caller root；`X-Session-Id` 由 OpenCode 自己发送。需要强 ledger/CAS 时再叠加 Remote tools/shim，不能把原生工具结果误写成 CAS 证明。
...
| Q3 | 目标 harness 能否稳定透传每次请求的身份 | OpenCode 1.17.18 exact Workspace 以 `X-Session-Id + 最新 user-turn canonical 前缀` 生成 candidate，并优先续用同 affinity 的唯一非终态 task；request key 绑定 harness session 与请求摘要，不含 task_id。Basic 仍独立。Codex/Claude 需要各自真实 profile 证据，不能套用 OpenCode 结论 |
