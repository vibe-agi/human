...
| 范围 | 已有证据 | 仍缺证据 |
|---|---|---|
| M0/M1 核心 | 三方言 stream/aggregate codec、持久循环、adapter/shim、Live Workspace/TUI；OpenCode 1.17.18 Basic 工具闭环及 exact Workspace `空镜像 → :pull → edit/result → bash/tasks → final → 同 session terminal 后下一 user turn 新 task` 已真实通过；Codex 0.144.5 Responses Basic 串行 `exec_command → function_call_output → final` 已真实通过 | OpenCode Live TUI 保存/auto-send 的完整人工验收；真实 harness 部分响应断网重试；Codex Workspace/Tasks/Live Workspace；Claude 真实工具闭环；外部产品任务 |
| 故障恢复 | 自动故障注入已覆盖 caller 同 key 5 次断流、worker 冷启动/5 次抖动/半开检测、gateway + SQLite 重启与三方重叠掉线；Workspace 正式测试再覆盖离线原生 edit、gateway/SQLite 与 worker outbox 重启、并发 replay、result continuation 与 save-ahead diff；Codex 0.144.4 黑盒已确认 500/响应前断 TCP 的 5 次重试与 turn metadata | 真实 OpenCode/Codex 的部分 SSE 恢复；超过 `max_pending` 的明确过期 UX；证书/凭据轮换；多 worker |
