...
> **产品线边界**：公共单机 `human local` 的参考人侧现在是公共 `workerkit` +
> `web.Server`，不是本章的 legacy completion TUI。本章保留 Bubble Tea 远程 gateway
> 产品面的既有规格，供后续决定迁移或独立维护；不能把 Web 的 exact profile 证据自动外推给
> legacy TUI。Web 已真实覆盖三端 Command/Tasks/Live Workspace 与
> Claude/Codex partial SSE；legacy TUI 的 Claude/Codex Tasks/Workspace 仍未实测。图片内联与
> IDE 快捷入口也尚未实现。
...
TUI 不启用终端鼠标跟踪，拖拽选择与复制继续由终端原生处理；滚动使用 `PgUp/PgDn`，不会为了鼠标滚轮占用普通选择事件。
...
- **一次发送手势**：若用户刚输入完时 worker state 的上一笔草稿快照仍在提交，本次 `Enter` / `Ctrl+R` / `Ctrl+D` 或 tool-call 发送会先排队；输入短暂锁定，最新草稿落盘后自动且只执行一次，无需再按第二次。等待期间 `Esc` 可取消并保留草稿；危险命令的第二次 `Enter` 仍是独立的人工授权确认。
...
Tasks 从当前请求历史里的 tool call/result 按 `tool_call_id` 恢复确认状态；本地编辑先形成草稿，再通过 caller 本轮声明的计划工具同步到客户侧 Agent 的 Tasks 视图。当前识别三种精确形态：

| caller | 工具 | 交互模型 |
|---|---|---|
| OpenCode 1.17.18 | `todowrite` | 全量 `todos[{content,status,priority}]` |
| Claude Code 2.1.217 | `TaskCreate` / `TaskUpdate` / `TaskList` | 逐任务生命周期，并从 result 恢复 task ID |
| Codex 0.145.0 | `update_plan` | 全量 `plan[{step,status}]`（可带 `explanation`） |

公共 Web 人侧的三端都已由固定版本真实 CLI + Playwright 从正式 Tasks 面板完成 pending→in_progress→completed continuation；Claude 还执行真实 `TaskList` 核对 completed 状态。Web 的 `PlanProfileResolver` 只对 exact harness/version 和完整 behavioral schema 启用，工具缺失、重复声明或 schema 漂移会关闭专用面板，而不是猜字段或生成 Human 自用 Todo；通用 declared-tool 编辑器仍可用。legacy TUI 若继续维护，必须迁到同一抽象并独立跑门，不能引用这份 Web 证据冒充已通过。

### 3.2 Command 是 caller 工具的快捷输入

公共 Web 人侧的 Command 通过可替换 `CommandProfileResolver` 启用：官方基础实现要求 exact harness/version 与完整 behavioral schema 匹配，分别映射 Claude Code 2.1.217 `Bash`、OpenCode 1.17.18 `bash`、Codex 0.145.0 `exec_command`。提交只生成 tool call，命令始终由客户 Agent 在它自己的权限系统、机器和当前 workspace 执行；Human UI/worker **绝不本地执行**，也不拿可能更宽的路由根覆写 cwd。三端成功和非零失败恢复均已由真实 CLI + Playwright 从正式 Web Command 面板跑通。命中危险词法会先要求确认；schema 不兼容时专用面板禁用，改用高级 declared-tool JSON。精确 `opencode@1.17.18` Workspace 的 `:pull path/to/file` 是 legacy TUI 的另一条受控能力：它生成 OpenCode 原生 `bash` 调用 `opencode debug file read --pure`，经 caller Agent 的正常权限闸返回 base64 精确字节并播种 mirror；空文件可 hydrate，前导 `-` 路径以 `./` 消歧。
...
**Human 工作目录**：`--workspace` 是接单人机器上的基础目录。每个 exact harness
session 默认得到稳定的 `session-<hash>` 子目录，也可在 Web 中切换到 Human 已有 repo。
人用自己的 IDE/Agent 在这里工作；递归 watcher 忽略 `.git`、debounce 后触发一次全量
review，因此编辑器的 rename-save/coalesced event 不会直接被当成增量真相。交付路径
相对于该 repo，由对方 Agent 在自己的 cwd 解析。
...
- **结果回流**：发送前先把 exact event 保存为 worker-state pending row，再把 reviewed mutation、exact tool-call ID/digest 与已发送内容作为 mirror delivery intent 持久化，最后以 `delivery_intent_recorded=true` 更新同一 exact pending payload；只有这次 phase 写入成功才允许进入 outbox。准备或确认失败会先在 mirror 写入 terminal discard tombstone 并移除 intent，再删除 pending row；重启只会继续尚未完成的准备/清理，不能把已放弃的旧 call 重新入队。只有匹配该 ID、call digest 与 profile result contract 的成功结果才把 caller baseline 推进到**已发送版本**。若人在等待 result 时又保存了更新草稿，新 diff 会相对新 baseline 继续留在 Review；只有没有 save-ahead 时 Review 才归零。真实 OpenCode 1.17.18 gate 已收到 `Edit applied successfully.` 并完成这一对账；这证明准确确认“发出的版本”，不等于 remote SHA/CAS 证明。
...
**S2 Live Workspace（核心流程，Agent 驱动、人监督）**：按 `a` 接单 → 若镜像为空则在 Command 用 `:pull path` 精确播种所需文件 → 在 REPLY 连续说明 → 编辑 TASKS 并 `Ctrl+S` 调客户计划工具 → 在镜像用 IDE 保存 → watcher fresh review → 默认 preview/confirm，或显式 auto-send 干净改动 → exact profile 生成原生 edit/write → 客户 Agent 执行并回传 result → 按 delivery intent reconcile baseline、保留任何 save-ahead diff → bash 验证 → handoff/final。OpenCode 1.17.18 的 `:pull/edit/result/bash/tasks/final`、同 session terminal 后下一 user turn 会形成新 task、原始键值驱动生产 TUI，以及 headers/stream-start/progress 三个真实断线点均已成为可重复发行门。
...
**caller 断线不是取消**：HTTP/SSE socket 消失不会撤销已经持久化的 admission；同 key 重试可在 `max_pending` 内续接。公共服务把断连绑定到 exact request identity，Web 显示 caller-gone 告警；只有该 exact active request 仍未完成时，新的 resume 才能接管，旧 handler 的迟到 cleanup 或仍在线 caller 都不能授权夺权。人可在 caller 已断连后显式结束会话，但单纯断网不会被伪报为“用户取消”。

**恢复**：本机 `human local` 由 `llm.Service` 持久化请求/事件/receipt，进程内 `workerkit` 直接提交，重启后由 service 重建 pending assignment；它没有 worker WebSocket/outbox。远程 `human gateway` + `human worker` 才使用 WebSocket 重连与 durable outbox。两条拓扑都维持 exact request/event 幂等，但证据边界不能混写。Codex 已补 Responses Basic 工具闭环、partial SSE 重试和 exact `apply_patch` Workspace 正常路径；Workspace 故障恢复仍需外部验证。恢复必须发生在 `max_pending` 剩余窗口内；超时后 public service 持久提交 `expired`，按 exact request 清理人侧状态，迟到回复得到稳定 NACK 且不会影响同 task 的更新 continuation。
...
若 gateway 因会话已过期而拒绝已进 outbox 的事件，worker 会在同一个 SQLite 事务里把原事件及脱敏 scope 移入有序 rejected inbox、同时应用累计 ACK；独立 dispatcher 可跨重启重放，直到 TUI 完成同一 scope 的 cleanup、pending delete、草稿安装并确认，之后只留 digest tombstone。此 finalizer 只阻止同一稳定 task/session scope 继续发送，无关请求仍可处理；被拒高级工具草稿按 `event_id + 行号` 确定性换一组新 call ID，绝不复用已丢弃 ID。Remote tools / Workspace 可按稳定 scope 跨 completion 恢复；Chat 草稿不因可伪造的 task_id 带入别的请求。镜像交付预览、正在编辑的高级 JSON composer 等**未列入 state DB 的瞬时 UI 状态**仍需重新建立，不能宣称整个屏幕任意时刻无损恢复；达到 32-scope 上限也会淘汰最旧项并明确提示。

## 8. 配置

经 `viper` 加载：只有显式 `--config` 指定的配置文件（toml/yaml/json）才与环境变量、flag 层叠；CLI 由 `cobra` 提供子命令。启动于客户 workspace 时不会自动读取其中的 `human.yaml`。
...
远程模式用 `human worker` 启动，worker token 通过 `HUMAN_GATEWAY_TOKEN` 或权限受限的 `--token-file` 注入，不写入配置文件或明文命令行参数；`worker.state_db=""` 可显式关闭 UI 恢复。`auto` 将 outbox/state 放入 OS 用户数据根（macOS `~/Library/Application Support/Human`、Linux `${XDG_DATA_HOME:-~/.local/share}/human`、Windows `%LOCALAPPDATA%\\Human`），绝对 `HUMAN_DATA_HOME` 可覆盖。本机模式用 `human local --workspace .`；workspace 会 realpath 并选择最近 Git root，public service DB、mode `0600` 的单 caller 凭据和 workerkit state 在用户数据根下按 workspace 摘要隔离，没有本地 worker token/outbox。`human local credentials --workspace . --token-only` 只在用户显式调用时输出 caller token。显式私有路径 flag 继续覆盖 `auto`，不提供旧 `.human` 开发库迁移。身份显示名、IDE 命令、通知和图片协议仍是产品规格，不应提前写入可运行样例。
