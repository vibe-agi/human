...
远程模式用 `human worker` 启动，worker token 通过 `HUMAN_GATEWAY_TOKEN` 或权限受限的 `--token-file` 注入，不写入配置文件或明文命令行参数；`worker.state_db=""` 可显式关闭 UI 恢复。`auto` 将 outbox/state 放入 OS 用户数据根（macOS `~/Library/Application Support/Human`、Linux `${XDG_DATA_HOME:-~/.local/share}/human`、Windows `%LOCALAPPDATA%\\Human`），绝对 `HUMAN_DATA_HOME` 可覆盖。本机模式用 `human local --workspace .`；workspace 会 realpath 并选择最近 Git root，gateway DB、mode `0600` 凭据、outbox/state 都在用户数据根下按 workspace 摘要隔离，SQLite 仍只保存 token hash。`human local credentials --workspace . --token-only` 只在用户显式调用时输出 caller token。显式私有路径 flag 继续覆盖 `auto`，不提供旧 `.human` 开发库迁移。身份显示名、IDE 命令、通知和图片协议仍是产品规格，不应提前写入可运行样例。
