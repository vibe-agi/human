...
humand 对外扮演 Anthropic / OpenAI 的模型 API，对内把请求交给人、把人的响应转回对应方言。它**不管理对话内容**（上下文每次由 harness 全量带来），但**必须管理跨 HTTP 回合的持久任务状态**——因为一次工作是多次 completion 请求的串（§5–6），"这个 tool_call 发出去了、结果还没回来"这类状态跨请求存活。故 humand 的同步面 = 三方言转换 + 准入/队列 + 心跳 + **持久任务状态机** + 审计；实现位于 `internal/completion/`。异步 A2A 权威独立位于 `internal/delegation/`，不复用 completion 状态机。
...
代码已覆盖 Chat + Remote tools 的核心边界及 Workspace 镜像/核对路径；产品承诺仍聚焦环境绑定型排障。Workspace 长研发档只有在 P1-M0 真实 harness 的 2h 门通过后才能对外承诺。
...
**Responses API 边界**：Responses request/stream codec 已作为第三 adapter 实现并有 golden 回归；这只证明项目自己的 wire 转换，不证明 Codex 或其他具体 harness 的 base_url、鉴权、代理链与重试行为兼容。故仍**不把“已实现 Responses codec”写成“已覆盖 Codex”**，具体 harness 由 P1-M0 实测矩阵裁决。
...
**canonical schema 版本化**：每次演进有版本号；转换器是纯函数。仓库已为三种方言加入脱敏代表性 request + 工具 schema + canonical golden，并由测试实际执行 decode→canonical→stream。真实 harness 抓取样本仍需在 P1-M0 脱敏后补入，不能用合成 fixture 替代外部兼容证据。
...
- **CLI**：`spf13/cobra`——`humand` 同时装配 completion gateway 与 delegation A2A；`human` 提供同步 TUI 和异步 delegation 子命令；`human-mcp` 已作为需求方本地的异步 MCP bridge 实现（[phase2](phase2-async-mode.md)）。
...
- `completion` 与 `delegation` 各自维护领域状态与表结构，可落在同一 SQLite 文件；它们共享基础设施而不混用状态机。delegation 的 A2A projection 以自己的 SQLite authority 为唯一真相。
