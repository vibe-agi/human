...
Human gateway 对外扮演 Anthropic / OpenAI 的模型 API，对内把请求交给人、把人的响应转回对应方言。人占据的是 **LLM 协议位置**，不是某个受限业务角色；上下文仍由 harness 带来，工具仍由客户 Agent 执行。Basic 把 completion 独立处理；Workspace / Remote tools 用受验证的稳定身份把多个 completion 接入持久任务与 Live Workspace（§5–6）。逻辑 gateway = 三方言转换 + 准入/队列 + 心跳 + 可选的**持久任务状态机** + 审计；本机由 `human local` 内嵌，远程由 `human gateway` 独立运行。

> 本篇吸收 codex review 的 P0 修正：接入分档、跨回合状态机、准入/响应模式错误边界、显式 adapter、稳定标识、默认安全。

> **实现状态**：本篇协议核心、SQLite 持久化、worker WebSocket、caller shim、Chat/Messages/Responses 的 stream/aggregate codec 与脱敏 golden fixtures 已有 Go 实现和仓库内测试。OpenCode 1.17.18 的 Basic 工具闭环及 exact Workspace 的真实 CLI `空 Human mirror → :pull 精确字节 → 原生 edit/result → bash + todowrite → final → 同 session terminal 后下一 user turn 新 task` 已通过。Codex CLI 0.144.5 也已在空 `CODEX_HOME` 下真实跑通 Responses 串行 `exec_command → function_call_output → final` 两轮闭环，覆盖 namespace functions 与 hosted `web_search` 的分类；这只证明 Basic/Chat 工具循环，不是 Codex Workspace profile。Workspace 三方故障正式测试还覆盖原生 edit 在 caller/worker/gateway 重叠离线时进入持久 outbox、gateway/SQLite 与 worker 重启、并发 caller 重放、result continuation 与 save-ahead reconcile。10m/2h 只作历史证据；当前验证重点是剩余真实 harness 的断线、崩溃与乱序恢复。Codex CLI 0.144.4 的响应前重试/turn metadata 证据仍独立保留；Claude 只有本机 codec/schema 证据。
...
**Codex 0.144.5 Responses 工具 gate（Basic/Chat）**：测试用精确版本的真实 `codex exec`、空 `CODEX_HOME`、`--ignore-user-config --ephemeral` 和只读 sandbox，不读写用户配置。首轮 assignment 实收 `parallel_tool_calls:false`、普通 `exec_command`、`multi_agent_v1::*` namespace functions 与 hosted `web_search`；Human 发一个 `exec_command` 后，Codex 在本机实际执行并于第二轮以相同 `call_id` 回传包含标记的 `function_call_output`，随后消费 Human final、exit 0。该 gate 冻结了当前 Responses Basic 文本/函数契约；它没有 Codex exact Workspace 身份、Tasks matcher 实测或 Live Workspace 文件 mapper，不能外推这些能力。
...
会改变工具约束或输出结构、但尚未进入 canonical/专家界面的控制字段一律 fail-closed：指定/强制/禁用工具、结构化输出、Responses 的非空 `previous_response_id`、非空顶层 `reasoning` 配置、`store:true`、非空 `include` 与 `max_output_tokens` 均拒绝。Responses 的显式串行/并行工具策略已经进入 canonical 并受 gateway/TUI 强制；真实 Codex 使用的 `store:false`、空 `include`、对象型 `client_metadata` 与字符串型 `prompt_cache_key` 作为无输出语义的 provider hints 显式校验后忽略。顶层 envelope 使用严格字段解析，未知字段不会静默降级。
...
- **Tasks 只是 caller Agent 的计划编辑器**：只在本次请求声明且完整匹配 `todowrite`、`TodoWrite` 或 `update_plan` 的全量列表 schema 时启用；从历史 tool call/result 按 ID 恢复计划，编辑后仍通过同一个 caller 工具同步。它与请求队列分离，也不会在无工具时生成 Human 自用 Todo 或伪装成文本同步。OpenCode 1.17.18 的 `todowrite` 已有真实 fixture/闭环；Claude `TodoWrite` 与 Codex `update_plan` 目前只有本机契约适配和仓库测试，尚无各自真实 harness e2e。
- **Command 只是声明工具的快捷编辑器**：仅在本次请求声明兼容 `bash`，或 Remote adapter 对当前任务显式授权 exec 时启用；`Enter` 生成 tool call，由客户 Agent 执行，Human TUI 绝不在本地执行该命令。
...
- **CLI**：`spf13/cobra`——`human local` 单进程运行 gateway/SQLite/TUI，`human gateway` 独立部署，`human worker` 只连接远程 gateway，`human shim` 是可选 caller 边界；顶层 `human` 只负责选择显式子命令。
...
- **公共库**：`gateway.Open` 返回 `http.Handler` 以及可分别挂载的 model/worker handler，不占 listener；请求级 authenticator 可读取 Cookie/JWT/mTLS/上游 context，`WorkerRouter` 用完整 request、已认证 caller、model、capability tier 与 workspace/task/harness 身份选择新任务的精确 worker subject。选择先于 queue reserve 并作为 `lease_owner` 持久化；同 task continuation 与恢复只认原 owner，不再次调用路由器。policy denial、router failure、selected worker unavailable 是三种独立的 fail-closed 结果。`worker.Open` 返回可嵌入的 Bubble Tea model；`local.Open` 组合 loopback listener、gateway、SQLite 和 worker。自行实现 worker WS 必须在同一进程重连间保持 `X-Human-Worker-Instance` 稳定；不同进程共用 token 时 incumbent 不会被顶替，challenger 有界退避等待旧连接释放。

**存储边界**：当前公共运行时只支持 SQLite。内部按 `TaskStore`、receipt、audit 等数据形态切分，方便以后在确有多实例需求时重新设计；这些 internal interface 不是向第三方承诺的稳定 driver API。下列多 driver 只表示未来决策空间，不是现有配置：
...
- 内部接口**按数据形态切分**：任务状态机需要事务/CAS，审计与 TTL 具有不同访问形态；未来 driver 必须以同一套崩溃/重放测试验收，不能只满足方法签名。
- **当前实现与公共承诺只有 SQLite**（单机零依赖）；postgres/mysql/redis 待多实例需求出现再设计。上表的非 sqlite 行不是当前可填入配置的值。
