...
var (
	ErrClientConfiguration      = errors.New("invalid HumanLLM remote worker configuration")
	ErrClientClosed             = errors.New("HumanLLM remote worker client is closed")
	ErrClientAuthentication     = errors.New("HumanLLM remote worker authentication was rejected")
	ErrClientConnectionConflict = errors.New("HumanLLM remote worker identity already has an active connection")
	ErrClientMessageTooLarge    = errors.New("HumanLLM remote worker message exceeds the configured wire limit")
	ErrClientProtocol           = errors.New("HumanLLM remote worker protocol failed")
)

const (
	defaultClientReadLimit    int64 = 16 << 20
...
// ReconnectBackoff is called after each transient connection failure. Attempt
// starts at one and resets after a session survives ReconnectResetAfter. The
// client caps the returned delay at ReconnectMaxDelay and rejects non-positive
// values. Implementations are called by one connection-loop goroutine.
type ReconnectBackoff interface {
	Delay(attempt uint32) time.Duration
}

type ReconnectBackoffFunc func(uint32) time.Duration

func (function ReconnectBackoffFunc) Delay(attempt uint32) time.Duration {
	return function(attempt)
}

// ClientConfig configures the official remote HumanLLM worker. Journal is the
...
type ClientConfig struct {
	...
	ReadLimit      int64
	// WriteLimit is the maximum complete event envelope admitted to the durable
	// outbox. Keep it aligned with the gateway transport's ReadLimit. The 16 MiB
	// default matches the official server default; custom deployments may raise
	// both sides up to MaxWireMessageBytes.
	WriteLimit          int64
...
func (config ClientConfig) resolve(ctx context.Context) (resolvedClientConfig, error) {
	...
	probe := llm.AuthenticatedWorker{WorkerID: config.Worker, SessionID: "configuration-probe"}
	...
	if nilReconnectBackoff(config.Backoff) {
		return resolvedClientConfig{}, fmt.Errorf("%w: reconnect backoff is typed nil", ErrClientConfiguration)
	}
	...
	if config.WriteLimit == 0 {
		config.WriteLimit = defaultClientReadLimit
	}
	...
	if config.ReadLimit < 1024 || config.ReadLimit > MaxWireMessageBytes {
		return resolvedClientConfig{}, fmt.Errorf("%w: read limit must be 1024..%d", ErrClientConfiguration, MaxWireMessageBytes)
	}
	if config.WriteLimit < 1024 || config.WriteLimit > MaxWireMessageBytes {
		return resolvedClientConfig{}, fmt.Errorf("%w: write limit must be 1024..%d", ErrClientConfiguration, MaxWireMessageBytes)
	}
	...
	if err := journal.Bind(ctx, JournalBinding{Gateway: config.Gateway, Worker: config.Worker}); err != nil {
...
// Client is a durable remote HumanLLM worker runtime. Assignments and Rejections
...
type Client struct {
	...
	presentAssignments map[llm.WorkerDeliveryID]JournalDigest
	presentRejections  map[llm.WorkerDeliveryID]JournalDigest
...
func NewClient(ctx context.Context, config ClientConfig) (*Client, error) {
	...
	client := &Client{
		...
		assignments: make(chan llm.WorkerAssignmentDelivery, resolved.NotificationBuffer),
		...
		presentAssignments: make(map[llm.WorkerDeliveryID]JournalDigest),
		presentRejections:  make(map[llm.WorkerDeliveryID]JournalDigest),
		accepting:          true,
	}
...
func (client *Client) Assignments() <-chan llm.WorkerAssignmentDelivery { return client.assignments }
...
func (client *Client) SendEvent(ctx context.Context, delivery llm.WorkerEventDelivery) error {
	if ctx == nil {
	...
	canonical, err := canonicalClientEvent(delivery)
	if err != nil {
		return err
	}
	if err := canonical.ValidateFor(llm.AuthenticatedWorker{
		WorkerID: client.config.Worker, SessionID: "send-validation",
	}); err != nil {
		return err
	}
	if err := client.validateEventWireSize(canonical); err != nil {
		return err
	}
	digest, err := digestJournalValue(canonical)
	if err != nil {
		return err
	}
	state, err := client.config.journal.PutEvent(ctx, JournalEvent{
		Digest: digest, Delivery: llm.CloneWorkerEventDelivery(canonical),
	})
	if err != nil {
		return &JournalError{Operation: "put event", Delivery: canonical.ID, Cause: err}
	}
	if state != JournalEntryPending && state != JournalEntrySettled {
		return &JournalError{Operation: "put event", Delivery: canonical.ID, Cause: ErrJournalCorrupt}
	}
...
// canonicalClientEvent projects arbitrary Go JSON values (for example int and
// json.RawMessage) onto the same UseNumber representation used by the wire and
// SQLite adapter. Every official Journal therefore sees one stable logical
// value independent of whether the worker restarts through SQLite or memory.
func canonicalClientEvent(delivery llm.WorkerEventDelivery) (llm.WorkerEventDelivery, error) {
	encoded, err := json.Marshal(delivery)
	if err != nil {
		return llm.WorkerEventDelivery{}, fmt.Errorf("%w: encode event: %v", ErrClientProtocol, err)
	}
	var canonical llm.WorkerEventDelivery
	if err := decodeStrictJSON(encoded, &canonical); err != nil {
		return llm.WorkerEventDelivery{}, fmt.Errorf("%w: canonicalize event: %v", ErrClientProtocol, err)
	}
	return canonical, nil
}

func (client *Client) validateEventWireSize(delivery llm.WorkerEventDelivery) error {
	message, err := newEnvelope(messageEvent, delivery)
	if err != nil {
		return fmt.Errorf("%w: encode event: %v", ErrClientProtocol, err)
	}
	encoded, err := json.Marshal(message)
	if err != nil {
		return fmt.Errorf("%w: encode event envelope: %v", ErrClientProtocol, err)
	}
	if int64(len(encoded)) > client.config.WriteLimit {
		return fmt.Errorf(
			"%w: encoded event is %d bytes, configured maximum is %d",
			ErrClientMessageTooLarge, len(encoded), client.config.WriteLimit,
		)
	}
	return nil
}
...
func (client *Client) ConfirmAssignment(ctx context.Context, delivery llm.WorkerDeliveryID) error {
	if ctx == nil {
		return fmt.Errorf("%w: context is required", ErrClientConfiguration)
	}
	if err := client.beginOperation(); err != nil {
...
func (client *Client) ConfirmRejection(ctx context.Context, delivery llm.WorkerDeliveryID) error {
	if ctx == nil {
		return fmt.Errorf("%w: context is required", ErrClientConfiguration)
	}
	if err := client.beginOperation(); err != nil {
...
func (client *Client) run() {
	...
	releaseCancel()
	if releaseErr != nil {
		terminal = errors.Join(terminal, fmt.Errorf("release HumanLLM worker journal: %w", releaseErr))
	}
	client.errMu.Lock()
	client.err = terminal
	client.errMu.Unlock()
...
func nilReconnectBackoff(backoff ReconnectBackoff) bool {
	if backoff == nil {
		return false
	}
...
func digestJournalValue(value any) (JournalDigest, error) {
	...
	if err != nil {
		return "", fmt.Errorf("encode HumanLLM worker journal value: %w", err)
	}
...
func newWorkerSessionID() (llm.WorkerSessionID, error) {
	var random [16]byte
	if _, err := rand.Read(random[:]); err != nil {
		return "", fmt.Errorf("create HumanLLM worker session identity: %w", err)
	}
	return llm.WorkerSessionID("session-" + hex.EncodeToString(random[:])), nil
}
