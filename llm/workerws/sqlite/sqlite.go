...
// HumanLLM remote-worker Journal port.
package sqlite
...
const (
	schemaVersion     = 1
	schemaFingerprint = "human-llm-worker-journal-v1-20260719c"
	databasePurpose   = "HumanLLM worker Journal database"

	// DefaultMaxPendingRecords and DefaultMaxPendingBytes gate each new
	// assignment or outbound event against all payload-retaining assignments,
	// events, and rejections. Settlement is never rejected by quota: an atomic
	// NACK move may temporarily add receipt/wrapper bytes above the configured
	// byte budget, after which no new payload is admitted until compaction brings
	// usage back below budget. Tombstones do not consume quota.
...
var (
	...
	ErrDatabaseInUse = errors.New("HumanLLM worker Journal database is already held by another live owner")
	...
	ErrUnsupportedSchema = errors.New("unsupported HumanLLM worker Journal sqlite schema; use a dedicated database")
)
...
func (config Config) withDefaults() (Config, error) {
	if config.MaxPendingRecords < 0 {
		return Config{}, errors.New("HumanLLM worker Journal MaxPendingRecords must not be negative")
	}
	if config.MaxPendingBytes < 0 {
		return Config{}, errors.New("HumanLLM worker Journal MaxPendingBytes must not be negative")
	}
	if config.MaxPendingRecords == 0 {
...
func Open(ctx context.Context, config Config) (framework.Resource[workerws.Journal], error) {
	if ctx == nil {
		return framework.Resource[workerws.Journal]{}, errors.New("open HumanLLM worker Journal sqlite: context is required")
	}
	...
	if err != nil {
		return framework.Resource[workerws.Journal]{}, fmt.Errorf("open HumanLLM worker Journal sqlite: %w", err)
	}
	closeDatabase := true
	defer func() {
		if closeDatabase {
	...
	if err != nil {
		return framework.Resource[workerws.Journal]{}, fmt.Errorf("own HumanLLM worker Journal sqlite Resource: %w", err)
	}
	closeDatabase = false
	releaseOwner = false
	return resource, nil
...
func (journal *journal) acquire(ctx context.Context) (func(), error) {
	if ctx == nil {
		return nil, errors.New("HumanLLM worker Journal operation requires a context")
	}
	if err := ctx.Err(); err != nil {
		return nil, err
	}
...
func (journal *journal) update(ctx context.Context, fn func(*sql.Tx) error) error {
	...
	if err := journal.commitTx(tx); err != nil {
		return errors.Join(workerws.ErrJournalCommitUnknown, fmt.Errorf("commit HumanLLM worker Journal transaction: %w", err))
	}
	return nil
}
...
func configureDatabase(ctx context.Context, database *sql.DB) error {
	...
	if err := database.QueryRowContext(ctx, "PRAGMA journal_mode = WAL").Scan(&journalMode); err != nil {
		return fmt.Errorf("configure HumanLLM worker Journal sqlite WAL: %w", err)
	}
	for _, pragma := range []string{
		...
		if _, err := database.ExecContext(ctx, pragma); err != nil {
			return fmt.Errorf("configure HumanLLM worker Journal sqlite: %w", err)
		}
	}
	return nil
}
...
func resolveDatabasePath(path string) (string, error) {
	...
	if path == "" {
		return "", errors.New("HumanLLM worker Journal SQLite path is required")
	}
	...
	if err != nil {
		return "", fmt.Errorf("resolve HumanLLM worker Journal database path: %w", err)
	}
	if err := os.MkdirAll(filepath.Dir(absolute), 0o700); err != nil {
		return "", fmt.Errorf("create HumanLLM worker Journal database directory: %w", err)
	}
	return absolute, nil
}
...
func requireCurrentOrEmptySchema(ctx context.Context, database *sql.DB) error {
	rows, err := database.QueryContext(ctx, `
		...
		ORDER BY name`)
	if err != nil {
		return fmt.Errorf("inspect HumanLLM worker Journal sqlite schema: %w", err)
	}
	var tables []string
	for rows.Next() {
		var table string
		if err := rows.Scan(&table); err != nil {
			rows.Close()
			return fmt.Errorf("inspect HumanLLM worker Journal sqlite table: %w", err)
		}
		tables = append(tables, table)
	}
	if err := rows.Close(); err != nil {
		return fmt.Errorf("inspect HumanLLM worker Journal sqlite tables: %w", err)
	}
	if len(tables) == 0 {
		return nil
	}
	...
	expected := []string{
		"llm_worker_journal_assignments_pending_idx",
		"llm_worker_journal_assignments",
		"llm_worker_journal_events_pending_idx",
		"llm_worker_journal_events",
		"llm_worker_journal_meta",
		"llm_worker_journal_rejections_pending_idx",
		"llm_worker_journal_rejections",
	}
...
func initializeSchema(ctx context.Context, database *sql.DB) error {
	...
	if err != nil {
		return fmt.Errorf("begin HumanLLM worker Journal schema transaction: %w", err)
	}
	defer tx.Rollback()
	if _, err := tx.ExecContext(ctx, journalSchema); err != nil {
		return fmt.Errorf("initialize HumanLLM worker Journal sqlite schema: %w", err)
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("commit HumanLLM worker Journal sqlite schema: %w", err)
	}
	return nil
}
...
CREATE TABLE IF NOT EXISTS llm_worker_journal_meta (
  ...
  CHECK(
    (binding_gateway_id IS NULL AND binding_worker_id IS NULL)
    OR
    (binding_gateway_id <> '' AND binding_worker_id <> '')
  )
);
INSERT INTO llm_worker_journal_meta (
  singleton, schema_version, schema_fingerprint, next_sequence
) VALUES (1, 1, 'human-llm-worker-journal-v1-20260719c', 0)
ON CONFLICT(singleton) DO NOTHING;

CREATE TABLE IF NOT EXISTS llm_worker_journal_assignments (
  delivery_id TEXT PRIMARY KEY CHECK(delivery_id <> ''),
  sequence INTEGER NOT NULL UNIQUE CHECK(sequence > 0),
  digest TEXT NOT NULL CHECK(length(digest) = 64),
  state TEXT NOT NULL CHECK(state IN ('pending', 'settled')),
  payload BLOB,
  CHECK(payload IS NULL OR (length(payload) > 0 AND length(payload) <= 67108864)),
  CHECK(
...
CREATE TABLE IF NOT EXISTS llm_worker_journal_events (
  delivery_id TEXT PRIMARY KEY CHECK(delivery_id <> ''),
  sequence INTEGER NOT NULL UNIQUE CHECK(sequence > 0),
  digest TEXT NOT NULL CHECK(length(digest) = 64),
  state TEXT NOT NULL CHECK(state IN ('pending', 'settled')),
...
CREATE TABLE IF NOT EXISTS llm_worker_journal_rejections (
  delivery_id TEXT PRIMARY KEY CHECK(delivery_id <> ''),
  sequence INTEGER NOT NULL UNIQUE CHECK(sequence > 0),
  ...
  state TEXT NOT NULL CHECK(state IN ('pending', 'settled')),
  payload BLOB,
  CHECK(payload IS NULL OR (length(payload) > 0 AND length(payload) <= 67174400)),
  CHECK(
    (state = 'pending' AND payload IS NOT NULL)
    OR (state = 'settled' AND payload IS NULL)
  ),
  FOREIGN KEY(delivery_id)
    REFERENCES llm_worker_journal_events(delivery_id)
    ON DELETE RESTRICT
);

CREATE INDEX IF NOT EXISTS llm_worker_journal_assignments_pending_idx
  ON llm_worker_journal_assignments(sequence)
  WHERE state = 'pending';
CREATE INDEX IF NOT EXISTS llm_worker_journal_events_pending_idx
  ON llm_worker_journal_events(sequence)
  WHERE state = 'pending';
CREATE INDEX IF NOT EXISTS llm_worker_journal_rejections_pending_idx
  ON llm_worker_journal_rejections(sequence)
  WHERE state = 'pending';
`
