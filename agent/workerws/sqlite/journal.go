...
func (journal *journal) Bind(ctx context.Context, binding workerws.JournalBinding) error {
	return journal.update(ctx, func(tx *sql.Tx) error {
		...
		result, err := tx.ExecContext(ctx, `
			UPDATE agent_worker_journal_meta
			SET binding_gateway_id = ?, binding_authority_id = ?, binding_worker_id = ?
			WHERE singleton = 1
			  AND binding_gateway_id IS NULL
			  AND binding_authority_id IS NULL
			  AND binding_worker_id IS NULL`,
			string(binding.Gateway), string(binding.Authority), string(binding.Worker),
		)
		if err != nil {
			return fmt.Errorf("persist Agent worker Journal binding: %w", err)
		}
		if count, err := result.RowsAffected(); err != nil {
			return fmt.Errorf("inspect Agent worker Journal binding update: %w", err)
		} else if count != 1 {
...
func (journal *journal) PutAssignment(
	...
	err := journal.update(ctx, func(tx *sql.Tx) error {
		...
		case !errors.Is(err, sql.ErrNoRows):
			return fmt.Errorf("load Agent worker Journal assignment: %w", err)
		}
...
func (journal *journal) ConfirmAssignment(ctx context.Context, id agent.WorkerDeliveryID) error {
	return journal.update(ctx, func(tx *sql.Tx) error {
		...
		if err := tx.QueryRowContext(ctx, `
			...
			return fmt.Errorf("load Agent worker Journal assignment settlement: %w", err)
		}
		...
		result, err := tx.ExecContext(ctx, `
			UPDATE agent_worker_journal_assignments
			SET state = 'settled', payload = NULL
			WHERE delivery_id = ? AND state = 'pending'`, string(id),
		)
		if err != nil {
			return fmt.Errorf("settle Agent worker Journal assignment: %w", err)
		}
...
func (journal *journal) ListAssignments(
	...
	if err != nil {
		return nil, fmt.Errorf("list Agent worker Journal assignments: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&deliveryID, &sequence, &digest, &payload); err != nil {
			return nil, fmt.Errorf("scan Agent worker Journal assignment: %w", err)
		}
		var delivery agent.WorkerAssignmentDelivery
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate Agent worker Journal assignments: %w", err)
	}
	return result, nil
}
...
func (journal *journal) PutEvent(
	...
	err := journal.update(ctx, func(tx *sql.Tx) error {
		...
		case err == nil:
			...
			return err
		case !errors.Is(err, sql.ErrNoRows):
			return fmt.Errorf("load Agent worker Journal event: %w", err)
		}
		...
		if _, err := tx.ExecContext(ctx, `
			INSERT INTO agent_worker_journal_events (
			  delivery_id, sequence, digest, state, payload, receipt_digest
			) VALUES (?, ?, ?, 'pending', ?, NULL)`,
			string(record.Delivery.ID), int64(sequence), string(record.Digest), payload,
		); err != nil {
			return fmt.Errorf("insert Agent worker Journal event: %w", err)
		}
		state = workerws.JournalEntryPending
		return nil
	})
...
func (journal *journal) ListEvents(
	...
	if err != nil {
		return nil, fmt.Errorf("list Agent worker Journal events: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var payload []byte
		if err := rows.Scan(&deliveryID, &sequence, &digest, &payload); err != nil {
			return nil, fmt.Errorf("scan Agent worker Journal event: %w", err)
		}
		var delivery agent.WorkerEventDelivery
		if err := decodePayload(payload, &delivery); err != nil {
			return nil, err
		}
		record := workerws.JournalEvent{
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate Agent worker Journal events: %w", err)
	}
	return result, nil
}
...
func (journal *journal) SettleEvent(
	...
	return journal.update(ctx, func(tx *sql.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("load Agent worker Journal event settlement: %w", err)
		}
		...
		result, err := tx.ExecContext(ctx, `
			UPDATE agent_worker_journal_events
		...
		if err != nil {
			return fmt.Errorf("settle Agent worker Journal event: %w", err)
		}
		...
		if receipt.Decision == agent.WorkerEventNACK {
			...
			if _, err := tx.ExecContext(ctx, `
				INSERT INTO agent_worker_journal_rejections (
				...
				return fmt.Errorf("insert Agent worker Journal rejection: %w", err)
			}
		}
		return nil
	})
...
func (journal *journal) ListRejections(
	...
	if err != nil {
		return nil, fmt.Errorf("list Agent worker Journal rejections: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&deliveryID, &sequence, &eventDigest, &receiptDigest, &payload); err != nil {
			return nil, fmt.Errorf("scan Agent worker Journal rejection: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate Agent worker Journal rejections: %w", err)
	}
	return result, nil
}

func (journal *journal) ConfirmRejection(ctx context.Context, id agent.WorkerDeliveryID) error {
	return journal.update(ctx, func(tx *sql.Tx) error {
		if _, err := requireBinding(ctx, tx); err != nil {
			return err
		}
		...
		if err := tx.QueryRowContext(ctx, `
			...
			return fmt.Errorf("load Agent worker Journal rejection settlement: %w", err)
		}
		switch workerws.JournalEntryState(state) {
		case workerws.JournalEntrySettled:
			return nil
		...
		result, err := tx.ExecContext(ctx, `
			UPDATE agent_worker_journal_rejections
			SET state = 'settled', payload = NULL
			WHERE delivery_id = ? AND state = 'pending'`, string(id),
		)
		if err != nil {
			return fmt.Errorf("settle Agent worker Journal rejection: %w", err)
		}
...
func inspectBinding(ctx context.Context, query rowQuerier) (workerws.JournalBinding, bool, error) {
	...
	if err := query.QueryRowContext(ctx, `
		SELECT binding_gateway_id, binding_authority_id, binding_worker_id
		FROM agent_worker_journal_meta
		WHERE singleton = 1`).Scan(&gateway, &authority, &worker); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return workerws.JournalBinding{}, false, corruptf("binding metadata is missing")
		}
		return workerws.JournalBinding{}, false, fmt.Errorf("load Agent worker Journal binding metadata: %w", err)
	}
	if !gateway.Valid && !authority.Valid && !worker.Valid {
	...
	binding := workerws.JournalBinding{
		...
		Authority: agent.AuthorityID(authority.String),
		Worker:    agent.WorkerID(worker.String),
	}
	if err := binding.Validate(); err != nil {
...
func requireBinding(ctx context.Context, query rowQuerier) (workerws.JournalBinding, error) {
	...
	if !bound {
		return workerws.JournalBinding{}, errors.Join(workerws.ErrJournalCorrupt, errors.New("Agent worker Journal is not bound"))
	}
	return binding, nil
}
...
func allocateSequence(ctx context.Context, tx *sql.Tx) (workerws.JournalSequence, error) {
	...
	if err := tx.QueryRowContext(ctx, `
		...
		return 0, fmt.Errorf("load Agent worker Journal sequence allocator: %w", err)
	}
	if current < 0 || current >= math.MaxInt64 {
		return 0, errors.Join(workerws.ErrJournalLimit, errors.New("Agent worker Journal sequence space is exhausted"))
	}
	next := current + 1
	result, err := tx.ExecContext(ctx, `
		UPDATE agent_worker_journal_meta
	...
	if err != nil {
		return 0, fmt.Errorf("advance Agent worker Journal sequence: %w", err)
	}
...
// requirePendingCapacity measures encoded payload bytes, not raw artifact
// bytes. It runs only for a genuinely new assignment/event and inside the same
// serializable transaction as insertion. Settlement and compaction deliberately
// bypass it so an over-budget Journal can always converge.
func (journal *journal) requirePendingCapacity(ctx context.Context, tx *sql.Tx, newPayloadBytes int64) error {
	...
	if err := tx.QueryRowContext(ctx, `
		...
		return fmt.Errorf("measure Agent worker Journal pending payloads: %w", err)
	}
...
func validateAssignmentDelivery(delivery agent.WorkerAssignmentDelivery, binding workerws.JournalBinding) error {
	err := delivery.ValidateFor(agent.AuthenticatedWorker{
		Authority: binding.Authority, Worker: binding.Worker, Session: "journal-validation",
	})
	if err != nil {
		return errors.Join(workerws.ErrJournalCorrupt, err)
	}
...
func validateEventDelivery(delivery agent.WorkerEventDelivery, binding workerws.JournalBinding) error {
	err := delivery.ValidateFor(agent.AuthenticatedWorker{
		Authority: binding.Authority, Worker: binding.Worker, Session: "journal-validation",
	})
	if err != nil {
...
func requireOneRow(result sql.Result, operation string) error {
	...
	if err != nil {
		return fmt.Errorf("inspect Agent worker Journal %s: %w", operation, err)
	}
	if count != 1 {
