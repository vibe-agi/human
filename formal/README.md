...
`HumanLLM`、`HumanAgent` 大安全模型和 `HumanSystem` 会再用 fingerprint 1 重跑。runner 共执行 73 个门：30 个正向检查（含三个 alternate fingerprint）、5 个指定环境/coverage 反例和 38 个 mutant。五个指定反例分别证明：无 Human 时不能保证进展；三方同时离线可达；五次故障预算确实可耗尽；不同 Context 可共享一个 Workspace；同一 Context 可并行 Task。

## Mutant oracle

`make-mutant.py` 会在临时目录生成 **38 个**故意错误的模型；runner 要求它们全部被指定 oracle 抓住：
...
- Go Agent 的 confirmed-head CAS 由**受信 caller receipt**驱动：领域库不观察真实文件树 `dirty`、不计算首个 base，也不执行 bundle。`HumanAgent.tla` 的 `~dirty`/exact apply guard 必须由未来 caller adapter 的 durable ledger、文件 CAS/journal 与 observed-result 检查来 refine；当前测试不能外推为真实多文件 apply 已完成。
- Go 已实现独立的 `human.NewAgent` 领域/SQLite、消息循环、Artifact 原子发布与 apply receipt CAS；A2A/worker transport、commit-time lease/fence grant 与 caller 侧 bundle journal 尚未实现，不能因领域构造器或 TLA 模型存在就宣称完整 HumanAgent 网络产品已经交付。
